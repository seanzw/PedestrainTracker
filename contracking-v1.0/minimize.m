function [X, fX, i, XX] = minimize(X, f, length, P1)
...
updOutput=10;   % print output every n iterations

% detections
% global detMatrices;
global detections sceneInfo opt gtInfo globiter cemStartTime allens allmets2d allmets3d

printMessage(2,'Conjugate Gradient Descent\n');
if sceneInfo.gtAvailable
    if opt.track3d
        printMessage(3,'\n --- INFO ---- | ------------  E N E R G Y    V A L U E S  -------- |||  ----------- M E T R I C S (3D)---------- |||  ----------- M E T R I C S (2D)--------- ');
    else
        printMessage(3,'\n --- INFO ---- | ------------  E N E R G Y    V A L U E S  -------- |||  ----------- M E T R I C S (2D)---------- |||');
    end
    printMessage(3,'\n               |                                                    |||                                           ||| ');
    
    if opt.track3d
        printMessage(3,'\n git| it|  time| Energy |  Edet  | Edyn | Eexc | Eapp | Eper | Ereg ||| MOTA  MOTP| GT  MT  ML|  FP   FN IDs  FM  ||| MOTA  MOTP| GT  MT  ML|  FP   FN IDs  FM|\n');
    else
        printMessage(3,'\n git| it|  time| Energy |  Edet  | Edyn | Eexc | Eapp | Eper | Ereg ||| MOTA  MOTP| GT  MT  ML|  FP   FN IDs  FM|\n');
    end
else
    printMessage(3,'\n --- INFO ---- | ------------  E N E R G Y    V A L U E S  --------------- ');
    printMessage(3,'\n               |                                                           ');
    printMessage(3,'\n git| it|  time| Energy |  Edet  | Edyn | Eexc | Eapp | Eper | Ereg |\n');
end
...
while i < abs(length)                                      % while not finished
  ...
  if success                                         % if line search succeeded
    ...
    %   O U T P U T   %
    %%%%%%%%%%%%%%%%%%%
    if ~mod(i,updOutput) || i==1
        
        
        if opt.visOptim
            prepFigure();
            plotDetections(1);
            plotTrajectories(X,P1,1); pause(.001);
%             saveas(gcf,sprintf('tmp/contoptim_%04d.png',globiter));
        end
...
%         fprintf('%s %6i;  Value %4.6e\r', S, i, f1);
        ...
        if opt.verbosity>=3
            [fx, ~, EdetValue EdynValue EexcValue EappValue EperValue EregValue] = eval(argstr);
            % weight energy values
            EdetValue=EdetValue*opt.weightEdet;
            EdynValue=EdynValue*opt.weightEdyn;
            EexcValue=EexcValue*opt.weightEexc;
            EappValue=EappValue*opt.weightEapp;
            EperValue=EperValue*opt.weightEper;
            EregValue=EregValue*opt.weightEreg;
            
            printMessage(3,'%4i|%3i|%6.1f|%8.1f|%8.1f|%6.1f|%6.1f|%6.1f|%6.1f|%6.1f|||', ...
                globiter,i, toc(cemStartTime)/60,fx,EdetValue,EdynValue,EexcValue,EappValue,EperValue,EregValue);
            allens(globiter,:)=[EdetValue EdynValue EexcValue EappValue EperValue EregValue];
        end
        
        if opt.verbosity >= 3 && sceneInfo.gtAvailable
...
%             stateInfo.X=Xm; stateInfo.Y=Ym;
            ...
            if opt.track3d
                ...
                printMetrics(metrics3d,metricsInfo3d,0,[12 13 4 5 7:11]);
...
%             printMetrics(metrics,metricsInfo,i==1);
            printMessage(3,'|||');
            printMetrics(metrics2d,metricsInfo2d,0,[12 13 4 5 7:11]);
